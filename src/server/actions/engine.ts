...
function triggerMatches(
  ...
  switch (trigger.type) {
    ...
    case "TIKTOK_GIFT":
      if (!ctx.giftName && ctx.giftCoins == null) return false;
      if (trigger.giftName && ctx.giftName) {
        return trigger.giftName.toLowerCase() === ctx.giftName.toLowerCase();
      }
      if (trigger.giftCoins != null && ctx.giftCoins != null) {
        return trigger.giftCoins === ctx.giftCoins;
      }
      return false;
    case "CHAT_COMMAND":
...
export async function runMatchedActions(params: {
  ...
  for (const action of matched) {
    ...
    const runAt = new Date(Date.now() + baseDelay + randomDelay);
