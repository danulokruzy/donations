...
export default function FakeDonationsPage() {
  ...
  async function addFake(e: FormEvent) {
    ...
    const res = await fetch("/api/fake-donations", {
      ...
      body: JSON.stringify({
        ...
        message: fakeMessage.trim() || "Донат!",
      }),
    });
    const data = await res.json();
    if (!res.ok || !data.ok) {
  ...
  function generateAutoFake() {
    ...
    let msgs: string[];
    try {
      msgs = JSON.parse(config.messages);
    } catch {
      msgs = ["Донат!", "Круто!", "Вперед!"];
    }

    const names = [config.name1, config.name2].filter(Boolean);
    const name = names[Math.floor(Math.random() * names.length)] || "Анонім";
    const amount = Math.floor(
      config.minAmountUah + Math.random() * (config.maxAmountUah - config.minAmountUah)
    );
    const msg = msgs[Math.floor(Math.random() * msgs.length)] || "Донат!";

    return { donorName: name, amountUah: amount, message: msg };
  }
